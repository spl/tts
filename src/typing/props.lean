...
→ typing (Γ₁ ∪ Γ₂) e t :=
  begin
    ...
    rw ←empty_union Γ₁ at ⊢,
    exact weaken_middle T (finset.disjoint_empty_left _) dj (finset.disjoint_empty_left _)
  end
  ...
  begin
    ...
    case typing.var : Γ y s' ts b ln_eq lts ls {
      ...
      { induction h,
        ...
        { simp at m, induction m, simp [weaken (Tex ln_eq lts) dj] },
    ...
    case typing.app : Γ ef ea t₁ t₂ Tf Ta ihf iha {
      exact app (ihf Γeq dj nmd @Tex lex) (iha Γeq dj nmd @Tex lex)
    },
    case typing.lam : v Γ eb t₁ t₂ L lt₁ Ft₂ ihb {
    ...
    case typing.let_ : v Γ ed eb sd tb Ld Lb _ _ ihd ihb {
      ...
      { intros xs ln_eq nd nm, apply ihd ln_eq nd (λ _ h, nmL_of_nm (nm _ h)) Γeq dj nmd @Tex lex },
      { induction Γeq, 
        intros y nm, 
        rw ←subst_open_var lex (ne_of_nm nm),
        rw ←insert_union,
...
→ typing Γ (subst x ex e₁) t :=
  begin
    ...
    rw ←empty_union Γ,
    exact subst_weaken T (finset.disjoint_empty_left _) (by simp [nm]) @Tex lex
  end
